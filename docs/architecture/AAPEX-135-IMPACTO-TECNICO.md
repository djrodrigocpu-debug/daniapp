...
| RPC | Mudança | Risco |
|---|---|---|
| `start_evaluation` | ✅ **vira WRAPPER (0047)**: o corpo vigente (0006 + 0031) é **movido** para `app.start_evaluation_legacy`. A guarda recusa `weekly` quando a data existe, **já venceu** e **não há ciclo semanal reaproveitável**. Enquanto nula, comportamento idêntico | baixo — o cutover nasce e permanece nulo. A terceira condição é o que impede rascunho órfão |
| `remove_evidence`, `reserve_evidence_upload` | ✅ **viram WRAPPER (0046)** para fechar o **O-18**. Recusa uniforme por escopo antes de delegar; corpo legado **movido**, não copiado | baixo — a autoria continua sendo decidida dentro do legado |
| `export_dataset` | ✅ **vira WRAPPER (0050)** para o Resumo definitivo (A-06). **Delega PRIMEIRO** e só então pós-processa o módulo `summary` | baixo — nada é executado antes da autorização da função movida |
| `get_monthly_audit_snapshot` | ✅ **vira WRAPPER (0051)** para parar de anunciar `A-10-pendente`. Mesmo padrão: delega primeiro | baixo |
| `get_monthly_audit_report_data` | ✅ **NOVA (0051)**. Só `monthly_criteria` **aprovada**, lendo **somente** o snapshot. Escopo antes de revelar existência | baixo |
...
| **RT-07** | CSV injection na exportação | 🟡 média | ✅ **MITIGADO (0049 + `csv.ts`/`xlsx.ts`)**. Neutralização por prefixo apóstrofo (OWASP) — que muda o **valor**, não a aparência — e a inspeção **pula o vazio à esquerda**: espaço, tabulação, quebra de linha e caractere de controle antes do sinal não escondem o ataque, porque a planilha também os ignora. Aplicada **somente** a colunas `type: 'text'`, e é por isso que **número negativo real continua número**. XLSX: **zero `<f>` e zero `hyperlink`**, medidos por varredura de todas as partes do zip; content-type de pasta **sem macros** |
| **RT-08** | Exportação virar caminho de contorno da RLS | 🔴 alta | ✅ **MITIGADO (0049)**. Ator, módulo, filtros e escopo resolvidos na porta única `export_dataset`; a geração do arquivo **não amplia** o conjunto devolvido. **Teste 35 na forma canônica**, nos quatro módulos: parceiro fora do escopo responde como o inexistente, zero linhas voltam com o erro, retrato de nove campos idêntico |
| **RT-14** | **Lista de filtro vazia lida como conjunto vazio** | 🟡 média | ✅ **MITIGADO, e nasceu de defeito real (0048)**. `p_filters ? 'statuses'` é verdadeiro para `statuses: []`, e o `in (select …)` sobre array vazio não casa com nada — **o painel esvaziava em silêncio**, parecendo "não há dado no período". Corrigido por `app.filter_len`: lista vazia é **ausência de filtro**. Recorrente por construção em todo filtro novo |
| **RT-15** | **Wrapper sobre wrapper** | 🟡 média | **novo, e permanente.** Cinco funções legadas já são wrapper (`submit_evaluation`, `get_official_audit_report_data`, `remove_evidence`, `reserve_evidence_upload`, `start_evaluation`). Cada camada nova roda **antes** da anterior, e a regra de O-16 vale a cada uma: verificar ator e escopo — ou delegar — **antes** de dizer qualquer coisa sobre o objeto |
| **RT-16** | **`score` anulável em `evaluations` e `official_snapshots`** | 🟡 média | **NOVO na Fase 10 (0050), e permanente.** As duas colunas eram `not null default 0`, e enquanto fossem, a decisão A-10 era inaplicável: "nenhum critério aplicável" virava nota zero **dentro do banco**. O CHECK por modelo que entrou é **mais forte** para o caminho legado. O risco que nasce é do outro lado: **todo consumidor novo que fizer `Number(score)` sem checar `null` produz zero** — exatamente o defeito que a mudança elimina. Defendido hoje em quatro camadas (banco, adapter, domínio, tela) e **recorrente por construção**. É a lição L-04 ganhando uma superfície |
