...
| `action_plans` | **estendida** | +`assisted_entry_id`, `monthly_audit_id`, `source` + CHECK de exclusividade |
...
| **RT-03** | `target_band` sem regra (A-01) | 🟡 média | A função **falha explicitamente**. Nunca escolher comportamento por conta própria |
| **RT-04** | Semana ISO vs. `week_start_date` divergindo por timezone | 🟡 média | `week_start_date` é `date`, calculado em `America/Sao_Paulo`; CHECK garante segunda-feira |
